...
# 3. Запуск фонового Telegram-демона 24/7 (работает независимо от терминала):
sluga start

# 4. Проверка статуса демона (работает/остановлен, PID, логи):
sluga status

# 5. Просмотр логов демона:
sluga logs

# 6. Остановка или перезапуск фонового демона:
sluga stop
sluga restart

# 7. Запуск бота прямо в консоли (для отладки перед глазами):
sluga bot

# 8. Провести экспресс-диагностику текущего проекта (МРТ):
sluga doctor

# 9. Запустить интерактивный мастер настройки окружения:
sluga setup
```
...
| `/balance` (или `/stats`) | Проверить остаток токенов, расход в рублях и лимиты LiteAI прямо в Telegram |
...
| `/voice on` / `/voice off` | Включить или выключить голосовые ответы агента в Telegram |
...
## 🎙️ Голосовое управление в Telegram (Voice Notes)

Агент SLUGA прямо из коробки при установке с GitHub оснащен **двусторонней голосовой связью**:

1. **Прием голосовых сообщений (Speech-to-Text):**
   - Просто зажмите микрофон в Telegram и надиктуйте любую задачу (например: *«Зайди в папку проекта и проверь код на ошибки»*).
   - Агент автоматически скачивает аудио, распознает русскую речь через Gemini Multimodal Audio / Whisper, подтверждает распознанный текст цитатой и запускает выполнение.
2. **Голосовой ответ агента (Text-to-Speech):**
   - Агент озвучивает суть выполненной работы живым русским голосом через встроенный движок **Microsoft Edge-TTS** (`ru-RU-DmitryNeural` — работает бесплатно, без ключей и подписок).
   - Голосовой ответ отправляется как голосовое сообщение (`.mp3`), а следом высылается подробный текстовый отчет с кодом и логами.
3. **Управление режимом:**
   - `/voice on` — включить голосовые ответы
   - `/voice off` — оставить только текстовые ответы
